...
public static class DisplaySums {
    ...
    private static void DisplaySumPairs(int[] numbers) {
        ...
        var tempSet = new HashSet<int>(numbers);
        
        foreach (int number in numbers)
        {
            if (tempSet.Contains(number) && tempSet.Contains(10 - number) && number != (10 - number))
            {
                Console.WriteLine($"{number} & {10 - number}");
                tempSet.Remove(number);
                tempSet.Remove(10 - number);
            }
        }
