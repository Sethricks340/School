...
public static class ArraysTester {
    ...
    public static void Run() {
        ...
        Console.WriteLine($"<List>{{{string.Join(',', numbers)}}}"); // <List>{2, 3, 1}
    }  
    /// <summary>
    ...
    private static void RotateListRight(List<int> data, int amount)
    {
        ...
        List<int> newData1 = data.GetRange( data.Count - amount, amount);
        List<int> newData2 = data.GetRange( 0, data.Count - amount);
        data.InsertRange(0, newData1);
        data.InsertRange(data.Count - amount, newData2);
