...
public static class MapsTester {
    public static void Run() {
        ...
        Console.WriteLine(IsAnagram("ABBBB", "BAAAA")); // false
        Console.WriteLine(IsAnagram("AA", "BB")); // false
    ...
    private static Dictionary<string, int> SummarizeDegrees(string filename) {
        ...
        foreach (var line in File.ReadLines(filename)) {
            ...
            //If the key isn't already in the Dictionary
            if (!degrees.ContainsKey(fields[3])) degrees.Add(fields[3], 1);
    ...
    private static bool IsAnagram(string word1, string word2) {
        ...
        //If they are the same word, they aren't anagrams
        if (word1.Equals(word2)) return false;
        //Count the letters in second word
        foreach (var character in word1)
        {
            string letter = character.ToString().ToLower();
            if (letter != " ")
            {
                if (!letters.ContainsKey(letter)) letters.Add(letter, 1);
        ...
        //If the number isn't even, they aren't Anagrams
        foreach (var letter in letters)
        {
            char character = letter.Key[0];
            //if one word contains it and the other doesn't, it's false
            if ((word1.ToLower().Contains(character) ^ word2.ToLower().Contains(character)) || letter.Value % 2 != 0) return false;
        }
        return true;
    }
    ...
    private static void EarthquakeDailySummary() {
        ...
        //1km NE of Pahala, Hawaii - Mag 2.36
        // 58km NW of Kandrian, Papua New Guinea - Mag 4.5
        // Print out each place a earthquake has happened today
        
        //For each feature collection, extract the place and magnitude
