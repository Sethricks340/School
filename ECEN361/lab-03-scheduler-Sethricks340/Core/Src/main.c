...
/******************************** STUDENT EDITABLE HERE STARTS HERE ***********************/
 void Scheduler_Dispatch(){
	...
	*/
	...
	//	for task in task array:
	for (int i = 0; i < num_tasks; i++){
		if (!list_of_tasks_to_do[i].suspended){
			// check remaining_time
			list_of_tasks_to_do[i].remaining_time -= runTime;
			// if it is not zero or negative, subtract by elapsed time
			if (list_of_tasks_to_do[i].remaining_time <= 0){
	...
	// if it is zero or negative, run task and then reset the remaining time
...
int main(void)
{
  ...
  HAL_Delay(1000);
	...
	Scheduler_StartTask(1000, D3_task);
	//
