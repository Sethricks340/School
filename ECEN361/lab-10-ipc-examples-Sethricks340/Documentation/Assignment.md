...
**The 'Semaphore_Toggle_task' waits for the the function 'StartDebounce to release the 'Button_1_SemaphoreHandle'. When the semaphore is released, the task toggles the LED.**
<br>
<br><br>

2.)	How long is the time between the button interrupt coming in and it being enabled again? <br>
**The delay is 30 milliseconds, 'StartDebounce' has osDelay(30);**
><br>
> <br>
...
**Yes, button one now toggles D4 and D3. BUT, not at the same time. This is because both of the tasks are waiting on a binary semaphore, which only wakes up one task. Because they are the same priority, they pass it to each other. If none of the LEDs are on, this is the pattern with four button clicks: D4 on, D3 on, D4 off, D3 off.**

4.)	Now change one of the priorities of these two tasks, re-compile,  and re-run.
How has the behavior changed?
**Now I have made the toggle LED 3 task a slightly higher priority. Because of this, LED 4 never gets toggled.**
...
**The display isn't changing up and down nearly as much, because of the new task. The '--' is seen a lot, blinking intermediately between numbers.**

>8.)	Is there a ‘priority’ associated with the Mutex?  If so, how can it be changed?

**Yes, it could be changed at this here:**
```c
...
><br> Did you see any effect on the ability of Button_3 to reset the count?

**It didn't have as big of an affect as I thought it would. I thought that it would be similar to the nature of the semaphores in part one, where one of the tasks would be starved due to the higher priority of the other. But in this case, not all the tasks are available. The task manager chooses the highest *available* task, which in this case is sometiems teh idle reset task. So the effect is that it slightly delays the reset, but only by a fraction of a second.**
>
---
<!--------------------------------------------------------------------------------->
...
**From the resource on CMSIS-RTOS, a timer made with osTimerNew has the possible parameters of 'osTimerOnce;, and 'osTimerPeriodic.' This means that the timer can repeat periodically or just be called once.**

>11).	The debounce for the switches here used an osDelay() call (non-blocking).  Is there any advantage to using a SWTimer here instead? 

**'osDelay' actually is blocking, but it just blocks the thread that called it until it expires. Not the entire processor. The advantage to using a software timer here instead would be that the thread would not be blocked, meaning it could run other code while this timer is running.**
...
**Because the previous timer had a prescaler of 800 and a period of 100, the new timer needs to have a period of 1 / (80MHz / (800 * 100)) = 5ms. Here is how I did it:**

```c
// Declare new timer parameters:
...
// new function does the 7seg stuff
