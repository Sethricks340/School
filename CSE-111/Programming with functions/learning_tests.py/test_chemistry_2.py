...
from chemistry import make_periodic_table, compute_molar_mass, make_known_molecules_dict, get_formula_name
...
def test_get_formula_name():
    #Assure that all the known formulas correspond with their correct names in the molecules dictionary
    assert get_formula_name("Al2O3", make_known_molecules_dict()) == "aluminum oxide"
    assert get_formula_name("CH3OH", make_known_molecules_dict()) == "methanol"
    assert get_formula_name("C2H6O", make_known_molecules_dict()) == "ethanol"
    assert get_formula_name("C2H5OH", make_known_molecules_dict()) == "ethanol"
    assert get_formula_name("C3H8O", make_known_molecules_dict()) == "isopropyl alcohol"
    assert get_formula_name("C3H8", make_known_molecules_dict()) == "propane"
    assert get_formula_name("C4H10", make_known_molecules_dict()) == "butane" 
    assert get_formula_name("C6H6", make_known_molecules_dict()) == "benzene"
    assert get_formula_name("C6H14", make_known_molecules_dict()) == "hexane" 
    assert get_formula_name("C8H18", make_known_molecules_dict()) == "octane"
    assert get_formula_name("CH3(CH2)6CH3", make_known_molecules_dict()) == "octane"
    assert get_formula_name("C13H18O2", make_known_molecules_dict()) == "ibuprofen"
    assert get_formula_name("C13H16N2O2", make_known_molecules_dict()) == "melatonin"
    assert get_formula_name("Fe2O3", make_known_molecules_dict()) == "iron oxide"
    assert get_formula_name("FeS2", make_known_molecules_dict()) == "iron pyrite"
    assert get_formula_name("H2O", make_known_molecules_dict()) == "water"
    assert get_formula_name("H3O", make_known_molecules_dict()) == "unknown compound"
    assert get_formula_name("", make_known_molecules_dict()) == "unknown compound"
    assert get_formula_name(" ", make_known_molecules_dict()) == "unknown compound"
