...
def main():
    ...
    extract_birthdate = lambda student: student[2]
    extract_first_name = lambda student: student[0]

    sorted_list = sorted(students_list, key= extract_birthdate)
    sorted_first_name_list = sorted(students_list, key= extract_first_name)

    print("New list with sorted dates: ")
    print_list(sorted_list)
    print()
    
    print("New list with sorted dates: ")
    print_list(sorted_first_name_list)

    def extract_birthmonth_and_day(student_list):
        birth_month_and_day = student_list[2][5:]
        return birth_month_and_day

    #sorted_list = sorted(students_list, key= extract_birthmonth_and_day())

    print()
    print("Sorted by month")
    sorted_birth_month_list = sorted(students_list, key= extract_birthmonth_and_day)
    print_list(sorted_birth_month_list)

    for student in sorted_birth_month_list:    
        if student[2][5:] == "03-13":
            print(student)

    #item = sorted(birthdates)
    #print(item)
...
##SAMPLE SOLUTION
# right 2020, Brigham Young University-Idaho. All rights reserved.
